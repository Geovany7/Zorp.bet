...
function playOnClick() {
    ...
    const randomRotation = Math.floor(Math.random() * 3000) + 960; // Rotação aleatória entre 360 e 3960 graus
    saveRotation += randomRotation;
    globalObjects.roleta.style.transition = "transform 4s ease-out"; // Transição suave

    globalObjects.roleta.style.transform = `rotate(${saveRotation}deg)`;
    ...
    setTimeout(() => {
        ...
        document.getElementById("msgGanhador").innerHTML = `Parabéns! Você ganhou <b class="markOrange">${boxGanhador.innerHTML}</b>`;
...
document.getElementById("btnPlay").addEventListener("click", playOnClick)
